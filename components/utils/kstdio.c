...
u_inline int skip_atoi(const char **s)
{
    int i = 0;
    while (_ISDIGIT(**s))
        i = i * 10 + *((*s)++) - '0';
...
u_weak int k_vsnprintf(char *buf, size_t size, const char *fmt, va_list args)
{
    ...
    int32_t field_width = 0;     /* width of output field */
...
int k_snprintf(char *buf, size_t size, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    n = k_vsnprintf(buf, size, fmt, args);
    va_end(args);
 ...
 * @return The number of characters actually written to buffer.
 */
int k_vsprintf(char *buf, const char *format, va_list arg_ptr)
{
    return k_vsnprintf(buf, (size_t) - 1, format, arg_ptr);
}
...
int k_sprintf(char *buf, const char *format, ...)
{
    ...
    n = k_vsprintf(buf, format, arg_ptr);
    va_end(arg_ptr);
